...
public interface DeliveryWalletRepository extends JpaRepository<DeliveryLog, Long> {
	...
	// method to find the day's earnings so far (JPQL) - use LocalDate to match entity.createdOn
	@Query("select coalesce(sum(o.totalAmount) * 0.25, 0) "
	...
	public double getEarnings(@Param("partnerId") Long partnerId, @Param("start") LocalDate start, @Param("end") LocalDate end);
