...
public class DeliveryWalletServiceImpl implements DeliveryWalletService {
	...
	public List<DeliveryTransactionsDto> getTransactions(Long deliveryPartnerId, int limit) {
		...
		List<com.backend.entity.DeliveryLog> logs = walletRepository.findByDeliveryPartnerIdOrderByCreatedOnDesc(deliveryPartnerId, PageRequest.of(0, pageSize));
		...
		for(com.backend.entity.DeliveryLog dl : logs) {
			...
			dto.setEarnings(dl.getOrder().getTotalAmount() * 0.25);
			txns.add(dto);
		}
		return txns;
	}
